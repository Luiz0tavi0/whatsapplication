...
class DispatchService:
    def __init__(
        self, session: AsyncSession, provider_dispatcher: ProviderDispatcher
    ):
        self.session = session
        self.dispatcher = provider_dispatcher

    async def send_to_contact(
        self, phone: str, lid: str | None, nome: str
    ) -> Dispatch | None:
        ...
        try:
            provider_name, message_id = await self.dispatcher.send_text(
                phone, lid, message
            )
